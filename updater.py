...
class PlaylistRepository:
    def get_playlist_from_config(self, playlist_to_mirror_config):
        ...
        return Playlist(
            playlist['id'],
            playlist['name'],
            self._get_tracks_from_playlist(playlist['tracks']))
    ...
    def get_playlists_with_name(self, name):
        ...
        return [
            Playlist(
                playlist['id'],
                playlist['name'],
                self._get_tracks_from_playlist(sp.playlist(playlist['id'])['tracks'])
            ) 
            for playlist 
    ...
    def get_liked_songs(self):
        ...
        return Playlist(
            None,
            'Liked Songs',
            self._get_tracks_from_playlist(saved_tracks)
        )

    def _get_tracks_from_playlist(self, playlist_tracks):
        result = spotipy_items_to_tracks(playlist_tracks['items'])
        while playlist_tracks['next']:
            print("fetching tracks...: {}".format(len(result)))
...
for config in [
    ...
    tracks_to_remove = [
        ...
        if track.id not in [copy_track.id for copy_track in mirror_playlist.tracks]]
    tracks_to_add = [
        track
        for track
        in mirror_playlist.tracks
        if track.id not in [existing_track.id for existing_track in playlist_to_populate.tracks]
    ]
    tracks_to_add.reverse()

    print("Tracks to remove: {}".format(len(tracks_to_remove)))
    print("Tracks to add: {}".format(len(tracks_to_add)))
    ...
    for i in range(0, len(tracks_to_add), 100):
        sp.user_playlist_add_tracks(
            current_user_id,
            playlist_to_populate.id,
            [track.id for track in tracks_to_add[i:i+100]])